...
function useAudio() {
  ...
  const down = useCallback(
    (key: string) =>
      play(KEY_SOUNDS_DOWN[key.toUpperCase()] || KEY_SOUNDS_DOWN[key]),
    [play],
  );
  const up = useCallback(
    (key: string) =>
      play(KEY_SOUNDS_UP[key.toUpperCase()] || KEY_SOUNDS_UP[key]),
    [play],
  );
...
function MatrixLine() {
  ...
  return (
    <span className="text-emerald-400 font-mono text-xs opacity-80">
      {frame}
    </span>
  );
}
...
function HackLine() {
  ...
  return (
    <span className="text-green-300 font-mono text-xs opacity-70 tracking-widest">
      {frame}
    </span>
  );
}
...
export function Terminal({
    ...
    (raw: string[] | OutputLine[]) => {
      const normalized: Line[] = raw.map((r) => {
        if (typeof r === "string")
          return r === ""
            ? { kind: "blank" as const }
            : { kind: "out" as const, text: r };
        return r.text === ""
          ? { kind: "blank" as const }
          : { kind: "out" as const, text: r.text, color: r.color };
      });
      addLines(normalized);
    },
    [addLines],
  ...
  useEffect(() => {
    ...
    setIntroCurrentText("");
    if (introOutputs.length > 0) {
      setIntroOutputIdx(0);
      setIntroPhase("outputting");
  ...
  // Done — show help then hand off to auto mode OR interactive
  useEffect(() => {
    if (introPhase !== "done") return;
    addLines([{ kind: "blank" }]);
    // Auto-display help output
    if (commandMap.help) {
      pushOutput(commandMap.help([]));
    }
    const t = setTimeout(() => {
    ...
    return () => clearTimeout(t);
  }, [introPhase, addLines, pushOutput, commandMap, autoCommands]);
  ...
  const runCommand = useCallback(
    (raw: string) => {
      ...
      if (cmd === "matrix") {
        addLines([
          { kind: "out", text: "  Entering the Matrix...", color: "green" },
        ]);
      ...
      if (
        ...
        addLines([
          ...
          {
            kind: "out",
            text: "  Removing /home/soham/projects...",
            color: "red",
          },
          ...
          {
            kind: "out",
            text: "  Just kidding 😄  Nice try though.",
            color: "amber",
          },
          { kind: "blank" },
        ]);
      ...
      if (trimmed === "git log") {
        addLines([
          {
            kind: "out",
            text: "  commit a3f91b2  (HEAD -> main)",
            color: "amber",
          },
          {
            kind: "out",
            text: "  Author: Soham Shirke <sohamshirke473@gmail.com>",
            color: "dim",
          },
          { kind: "out", text: "  Date:   Today", color: "dim" },
          {
            kind: "out",
            text: "      fix: finally got that bug after 3 hours",
            color: "default",
          },
          { kind: "blank" },
          ...
          {
            kind: "out",
            text: "      feat: add feature (it works on my machine)",
            color: "default",
          },
          { kind: "blank" },
          ...
          {
            kind: "out",
            text: "      WIP: please don't look at this commit",
            color: "default",
          },
          { kind: "blank" },
          ...
          {
            kind: "out",
            text: "      init: copied from stackoverflow",
            color: "dim",
          },
          { kind: "blank" },
        ]);
      ...
      if (cmd === "neofetch") {
        addLines([
          ...
          {
            kind: "out",
            text: "  Host:     Thadomal Shahani Engineering College",
            color: "default",
          },
          {
            kind: "out",
            text: "  Kernel:   B.E Computer Engineering",
            color: "default",
          },
          {
            kind: "out",
            text: "  Uptime:   2+ years coding",
            color: "default",
          },
          {
            kind: "out",
            text: "  Shell:    TypeScript / Node.js",
            color: "default",
          },
          { kind: "out", text: "  Terminal: This one 😄", color: "default" },
          {
            kind: "out",
            text: "  CPU:      Curiosity × Caffeine",
            color: "default",
          },
          {
            kind: "out",
            text: "  Memory:   Infinite (if it's interesting)",
            color: "default",
          },
      ...
      if (cmd === "ping") {
        ...
        addLines([
          {
            kind: "out",
            text: `  PING ${target} (sohamshirke473@gmail.com)`,
            color: "dim",
          },
          {
            kind: "out",
            text: "  64 bytes: icmp_seq=1 ttl=64 time=0.001 ms",
            color: "green",
          },
          {
            kind: "out",
            text: "  64 bytes: icmp_seq=2 ttl=64 time=0.001 ms",
            color: "green",
          },
          {
            kind: "out",
            text: "  64 bytes: icmp_seq=3 ttl=64 time=0.001 ms",
            color: "green",
          },
          { kind: "blank" },
          {
            kind: "out",
            text: "  Always available. No packet loss. 🟢",
            color: "green",
          },
          { kind: "blank" },
        ]);
      ...
      if (cmd === "yes") {
        ...
        addLines([
          { kind: "out", text: "  ^C (killed)", color: "red" },
          { kind: "blank" },
        ]);
        return;
      }
      ...
      if (cmd === "exit" || cmd === "logout") {
        addLines([
          ...
          {
            kind: "out",
            text: "  Connection to soham.dev closed.",
            color: "dim",
          },
          { kind: "blank" },
          {
            kind: "out",
            text: "  ...psych. You can't leave 😄",
            color: "amber",
          },
          { kind: "blank" },
        ]);
      ...
      <div className="flex h-full flex-col overflow-hidden rounded-lg border border-neutral-800 bg-neutral-900 shadow-2xl">
        ...
        >
          {lines.map((line, i) => {
            ...
            // output
            const cls = line.color
              ? COLOR_CLASS[line.color]
              : "text-neutral-400";
            return (
              // biome-ignore lint/suspicious/noArrayIndexKey: stable append-only log index
          ...
          {autoMode && autoPhase === "pausing" && (
            <>
              <div className="whitespace-pre-wrap">
                ...
                <span
                  className={cn(
                    "inline-block h-4 w-2 bg-amber-400 align-middle",
                    !cursorOn && "opacity-0",
                  )}
                />
              </div>
          ...
          {!interactive &&
            (introPhase === "done" ||
              ...
              <div className="whitespace-pre-wrap">
                ...
                <span
                  className={cn(
                    "inline-block h-4 w-2 bg-neutral-300 align-middle",
                    !cursorOn && "opacity-0",
                  )}
                />
              </div>
